...
for wave in ("reward", "reward_topup", "entropy"):
    for r in csv.DictReader(open(f"docs/completion/results/{wave}_eval.csv", encoding="utf-8")):
...
claims = {
    "w10s-obj-cur": 1.295, "w10s-dlt-cur": 1.306, "w10s-shp-cur": 1.278,
    "w10s-obj-off": 1.175, "w10s-dlt-off": 1.163, "w10s-shp-off": 1.185,
}
...
print(f"  pooled SD data {pooled:.4f}  doc 0.0516 -> {'OK' if abs(pooled - 0.0516) < 0.0006 else '!!'}")
...
for r in rows:
    key = (
        "obj-cur" if ("obj-cur" in r[0] or "ent003" in r[0])
...
print(f"  checkpoints beating humans: {sum(1 for v in groups.values() for x in v if x[0] > 0.672)}/18")
print(f"  score range: {min(x[0] for v in groups.values() for x in v):.3f}"
